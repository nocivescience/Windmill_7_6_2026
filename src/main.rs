...
fn main() -> Result<(), String> {
    let sdl_context = sdl2::init()?; // Inicializar SDL2
    let video_subsystem = sdl_context.video()?; // Inicializar el subsistema de video
    let ttf_context = sdl2::ttf::init().map_err(|e| e.to_string())?; // Inicializar SDL2_ttf para renderizado de texto
